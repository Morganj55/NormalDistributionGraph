...
namespace BuisnessLogic_and_Data
{
    public class Presenter : IObserver<ModelUpdate>
    {
        ...
        {
            bool senderIsButton =  sender is Button;
            if (senderIsButton)
            {
        ...
        //Button Parent method 
        public void GenerateNormalDistributionCurve()
        {
            SetCustomControlProperteis();
            _view._graphXYArray = _model.GenerateCurve();
            _view._graphSDLines = _model.GenerateSDLines();
            _view._XValues = _model.XArray;
            _view.mean = _model.mean.ToString("0");
            _view.stdDev = _model.stdDev;
            _view.PaintCurve();
            _model.Reset();
        }
